 ...
 */
#define ARM_CONTEXT_GPR 30
#define ARM_FP_REGISTER 13
#define ARM_LR_REGISTER 30
...
typedef struct tgt_context_t
{
	uint64_t sp;
	uint64_t lr;
    uint64_t gpr[ARM_CONTEXT_GPR];
	uint64_t spsr_el1;
} tgt_context_internal_t;
#pragma pack(pop)

typedef void arm_vec_handler_t(arm_vec_t type, mem_t contextp);
