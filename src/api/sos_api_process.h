...
error_t sos_create_process(
		...
		uint32_t flags,
		sos_process_t * process) TINKER_API_SUFFIX;
...
error_t sos_create_thread(
		...
		uint32_t stack,
		uint32_t flags,
		sos_thread_t * thread) TINKER_API_SUFFIX;

error_t sos_get_thread_object(sos_thread_t * thread) TINKER_API_SUFFIX ;

error_t	sos_get_thread_priority(
		sos_thread_t thread,
		uint8_t * priority) TINKER_API_SUFFIX;

error_t sos_exit_thread(void) TINKER_API_SUFFIX;

void sos_debug(const char * const str) TINKER_API_SUFFIX;
